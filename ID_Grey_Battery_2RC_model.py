...
def battery_2rc_jax(t, x, args):
    R0, R1, C1, R2, C2, n,  u_interp = args
    u = u_interp.evaluate(t)
    dx  = [-n*u/3440.05372, -1/R1/C1*x[1]+1/C1*u,-1/R2/C2*x[2]+1/C2*u]
    dx = jnp.array(dx)
    return dx

term = ODETerm(battery_2rc_jax)
solver = Dopri5()
...
def create_loss_and_constraint_fns(t_shots, y_data, u_interp):
    ...
    def objective_jax(decision_vars):
        R0, R1, C1, R2,C2, n = decision_vars[:n_params]
        x_initial_shots = decision_vars[n_params:].reshape(n_shots, n_states)
        args = (R0,R1,C1,R2, C2, n, u_interp)
        ...
        def model_output_step(t, x_step, R0_scalar, u_interp_obj):
            ...
            y_pred_step = OCV + R0_scalar * u + x_step[1] + x_step[2]
            return y_pred_step
...
x_initial_first_shot = jnp.array([0.98, 0.0,0.0]) # SOC em 98%, Vc em 0V
...
b_no_limit = (None, None, None) #number of states
state_bounds_one_shot = [b_no_limit, b_no_limit, b_no_limit] # number of states
...
print(f"Estimated parameters: R0 = {R0:.4f}, R1 = {R1:.4f}, C1 = {C1:.4f}, R2 = {R2:.4f}, C2 = {C2:.4f}, n = {n:.4f}")
...
sim_results = {
    'Params' : np.array([R0,R1,C1,R2,C2,n]),
